...
- 首次引导：第一次进入 WebUI 会出现简短引导，帮助用户先改密码、看懂工具层和模块层。
...
- 官方功能模块：日记、媒体、人物印象。
- 官方外观模块：小窝 WebUI，以及纸庭、玻璃小屋、夜间工作室三套内置全局外观。
- 自定义完整模块：可以替代或扩展官方模块。
...
官方外观不是写死在页面里的皮肤，而是可在“设置 → 外观设置”里切换的真实模块。`0.5.14` 内置的三套外观分别偏向长期阅读、通透管理和夜间维护；用户自己的主题应放进 `framework/user_custom/webui/`，避免插件更新覆盖。
